 ...
 * 原始代码中多处调用 memcpy，Detector 的 buffer-overflow 规则会标记
...
void SafeCopy_copy(SafeBuffer *dst, const void *src, size_t n) {
    ...
    memcpy(dst->ptr, src, n);  // ← Detector 会标记这一行: CWE-120 buffer-overflow
...
size_t SafeCopy_strcpy(SafeBuffer *dst, const char *src) {
    ...
    memcpy(dst->ptr, src, len);  // ← 同样会被标记，但 SafeCopy 保证安全
    ((char *)dst->ptr)[len] = '\0';
    return len;
}
...
void process_user_data_unsafe(const char *user_input) {
    char buf[64];
    memcpy(buf, user_input, strlen(user_input));  // ← 真漏洞: 无 bounds check
