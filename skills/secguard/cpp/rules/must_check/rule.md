...
对每个预筛信号（匹配 `memory.must*` 的信号）：
1. 读取调用点源码（±15 行）
2. 确认调用点真实存在于扫描范围内
3. 确认该函数属于已知的 must-check 函数集合（malloc/calloc/realloc/mmap/reallocarray/fopen/socket/accept/snprintf/vsnprintf/fgets/fread/fwrite/read/write/setenv）
4. 按 Scenario 分类：内存分配 → S1，IO 资源获取 → S2，数据传输 → S3
...
| 环节 | 说明 |
|------|------|
| **Source** | 分配/IO/数据传输函数的返回值（malloc 的指针、fopen 的 FILE*、read 的 ssize_t 等） |
| **Propagate** | 返回值被赋给变量后，该变量在后续代码中是否被检查（NULL 比较、错误值比较、字节数检查） |
| **Sink** | 未检查的返回值被直接用于解引用、条件分支或第二行语句执行 —— 意味着返回值被丢弃或未验证 |
...
1. 按行号分组，检查信号间依赖（如 integer_overflow 绕过 → must_check 失效）
