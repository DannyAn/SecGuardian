...
### Q1: 命令字符串是否包含外部输入 (argv/stdin/网络/文件)?

**Yes** = 缺陷在此上下文中真实存在，有具体代码锚点
**No**  = 缺陷不成立——此调用点不满足缺陷触发条件

### Q2: 输入是否可被攻击者控制嵌入命令分隔符 (; | && $() ``)?

**Yes** = 攻击者可控制触发条件或输入
**No**  = 实际运行中不可达或不可控

### Q3: 是否使用了参数化 API (execve/fork+exec) 或经过了白名单验证?
...
```json
"judgment_matrix": {
    "Q1_external_input": true|false,
    "Q2_delimiter_control": true|false,
    "Q3_parameterized_or_whitelist": true|false,
