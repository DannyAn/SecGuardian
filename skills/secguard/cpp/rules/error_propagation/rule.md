...
如果检测到调用点在该函数内直接使用返回值前缺乏检查，但上层 caller 在调用前已经验证了前置条件（传参前提是文件已打开、内存已分配），则 Step 4 可以补充证据。具体处理见 [cross-function.md](references/cross-function.md)。
