...
### Q1: 资源获取 (fopen/socket/open) 是否存在没有匹配 close/release 的代码路径?

**Yes** = 缺陷在此上下文中真实存在，有具体代码锚点
**No**  = 缺陷不成立——此调用点不满足缺陷触发条件

### Q2: 缺少释放的代码路径在正常运行时是否实际可达 (错误路径/提前返回/异常)?

**Yes** = 攻击者可控制触发条件或输入
**No**  = 实际运行中不可达或不可控

### Q3: RAII 包装器、scope guard (scope_exit/__attribute__((cleanup))) 或 goto cleanup 是否管理此资源?
...
```json
"judgment_matrix": {
    "Q1_leak_path": true|false,
    "Q2_reachable": true|false,
    "Q3_managed": true|false,
