...
3. 按 Scenario 分类：
   - `fopen/open/openat/freopen/tmpfile` → Scenario 1（资源泄漏）
   - `socket/accept` → Scenario 1（资源泄漏）
   - `close/fclose` 后续有第二次 close → Scenario 2（双重关闭）
   - `close/fclose` 后续继续使用 → Scenario 3（关闭后使用）
...
| 环节 | 说明 |
|------|------|
| **Source** | `fopen(path, "r")` / `open(path, flags)` / `socket(...)` — 创建点 |
...
当 alloc 变量作为参数传递给其他函数时，追踪被调用函数内部是否负责关闭该资源。深度限制 1 层。对于 Scenario 2（双重关闭）和 Scenario 3（关闭后使用），同样适用于跨函数场景：
- Scenario 2: callee close(fd) → caller 也 close(fd)
- Scenario 3: callee close(fd) → caller 继续使用同一 fd
...
1. 按行号分组，检查信号间依赖（如同一 fd 先泄漏再 use-after-close）
