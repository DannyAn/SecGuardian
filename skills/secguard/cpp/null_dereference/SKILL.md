...
1. **事实校对**: 分配点的返回值是否确实被赋值给了变量？
2. **因果闭环**: 从分配到解引用的路径是否无条件可达？
3. **寻找豁免**: 是否在 xmalloc 包装中（已封装检查）？是否在 assert 之后？
4. **根因归并**: 同分配点的多个解引用 → 合并为 1 个 finding
5. **保守定性**: 不可达分支或条件保护下 → 降级；无保护直达 → 按严重度定级
