...
for lang_dir in cpp-vuln-demo python-vuln-demo java-vuln-demo go-vuln-demo; do
    ...
    python3 -c "import json; f={'schema_version':'1.0','finding':{'severity':'Critical','cwe':'CWE-120','detector':'memory.buffer-overflow','file':'test.$LANG_NAME','line':10,'function':'test','title':'Test','fix_summary':'Fix','partialFingerprints':[{'algorithm':'SHA-256','value':''}],'location':{'file_path':'test.$LANG_NAME','start_line':10,'end_line':12,'snippet':'bad'},'evidence':{'code_context':'bad','judgment_rationale':'no bounds','data_flow_path':'overflow'},'impact':{'attack_scenario':'overflow'},'fix':{'before_code':'old','after_code':'new'}}}; open('$FINDINGS_DIR/memory/buffer-overflow/44ac6b74b2cd_test-10.json','w').write(json.dumps(f,indent=2))" 2>/dev/null
