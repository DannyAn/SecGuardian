...
| 场景 | 排除依据 | 证据要求 |
|------|---------|---------|
| 使用 `mongo-sanitize` / `express-mongo-sanitize` 中间件 | 已过滤操作符 | 确认中间件已全局注册或在该路由/端点生效 |
| 显式类型校验 (`typeof input === 'string'`) | 已拒绝 Object 类型 | 确认在查询执行前对输入进行了 typeof 检查 |
| Mongoose Schema 类型强约束 (Number/String) | Schema 层保护 | 确认 Schema 定义中字段类型为具体标量类型，非 Mixed/Object |
| 查询字段来自内部枚举/常量 | 非用户输入 | 确认查询条件中的字段名和值均来自代码常量或内部枚举 |
| `$regex` 配合严格白名单正则 | 有输入限制 | 确认正则表达式来源为硬编码，且限制了输入字符类和长度 |
...
User\.(find|findOne)\(req\.query
\.find\(.*\.body\)|\.findOne\(.*\.body\)
→ MUST: code_context (查询语句及输入来源的完整代码)
→ MUST: judgment_rationale (用户输入是否作为查询操作符传递，是否经过类型约束或清洗)
...
→ (mongo-sanitize|express-mongo-sanitize|mongoSanitize)        # 使用清洗中间件
→ typeof\s+\w+\s*===\s*['"]string['"]                            # 显式类型校验
→ Schema\.Types\.(String|Number|Boolean|Date|Buffer)              # Mongoose Schema 强类型
→ \$regex\s*:\s*\/\^\[a-zA-Z0-9                                     # 白名单正则限制
→ find\(\{\s*\w+\s*:\s*[\w.]+\s*\}\)                              # 字段值来自内部常量
```
