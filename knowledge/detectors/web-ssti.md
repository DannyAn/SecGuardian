...
| 场景 | 原因 |
|------|------|
| `render_template("page.html", user=name)` | 静态模板 + 变量 |
| `Template("internal_template_str")` 不含外部输入 | 硬编码内部模板 |
| Jinja2 `SandboxedEnvironment` | 沙箱限制 |
| 模板内容来自数据库白名单（非用户直接输入） | 间接但安全 |
| 模板引擎默认自动转义（html/template）且仅用于 HTML | 仅 XSS 风险，非 SSTI |
