...
| 场景 | 原因 |
|------|------|
| Jackson `@JsonIgnore`/`@JsonProperty(access=READ_ONLY)` | 已排除 |
| GORM `json:"-"` tag | 已排除序列化 |
| DRF `fields = ['username', 'email']` 显式列表 | 白名单 |
| Mongoose `.select('-password')` / `.select('username email')` | 显式选择 |
| 管理后台/内部 API（有权限校验） | 内部使用 |
| OAuth token 响应（标准协议） | 协议要求 |
