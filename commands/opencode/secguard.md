...
**读取上下文人命令（只需少量有限次 bash 调用，按 Signal 文件分组）:**

```bash
# 按文件分组的读取：每个出现信号的源码文件只读一次
# 对每个包含 Signal 的文件，读信号行 ±3 行
cat "$SOURCE_DIR/src/file.c" | sed -n '97,103p'   # Signal at line 100
```
...
3. 收集所有 blindspot 报告，汇总到 `worker_manifest.json`:
