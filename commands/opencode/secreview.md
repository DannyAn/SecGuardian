---
name: secreview
description: "[OpenCode] AI Security Code Review — 5-language PR security review via EPIC-3 Dispatcher protocol"
platform: opencode
---
...
## ⚠️ Context Budget Warning (OpenCode Hard Constraint)

> OpenCode has no background tasks — all review passes run serially in the main session.
>
...
```bash
source "$HOME/.config/opencode/extensions/secguardian/scripts/init-scan.sh" secreview "<path>" "<language>"
```
...
if command -v timeout &>/dev/null; then
    ...
    echo "WARNING: 'timeout' not found — indexer runs without timeout limitation"
...
### Step 5: Output Structured Findings (Findings Protocol v5.0 — File-First)
...
**5a. ⭐ 唯一 canonical 记录流程**

> **Detector name format**: `{namespace}.{name}` (e.g., `web.sql-injection`, `crypto.hardcoded_secrets`, `memory.buffer_overflow`).
> Consistent across all detection rules. No bare names.
