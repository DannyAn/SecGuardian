...
> 用 `scripts/record-finding.py` 记录每个 finding（禁止自己写脚本到 `/tmp/`）：

```bash
# 对每个检测器：读取其 .md → 分析源码 → 记录 finding
python3 scripts/record-finding.py \
    ...
    --snippet "<code>" \
    --fix-before "<bad>" --fix-after "<good>"
```

**必须**：对每个匹配的检测器，读取其 `.md` 文件，分析源码，然后调用 `scripts/record-finding.py` 记录。
**禁止**：创建 Python/Shell 脚本到 `/tmp/` 来批量生成 findings。
