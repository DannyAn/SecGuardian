...
| 场景 | 排除依据 | 证据要求 |
|------|---------|---------|
| `open()` + `O_NOFOLLOW` | 已禁止跟随 | 确认 open flags 包含 O_NOFOLLOW |
| `lstat()` 代替 `stat()` | 不跟随 symlink | 确认使用 lstat 而非 stat 获取文件信息 |
| `unlink()` 删除 symlink 本身 | symlink 被安全处理 | 确认使用 unlink 而非 remove 操作路径 |
| 目录为 root 所有且不可写 | 攻击者无法创建 symlink | 确认文件路径所在目录为 root 所有且权限为 755 或更严格 |
...
→ 路径可能来自外部
                                                       # → MUST: code_context (open 调用+flags)
...
→ 后续基于不完整信息做决策
                                                       # → MUST: judgment_rationale (路径来源+跟随风险)

# === EXCLUDE (不报告) ===
→ O_NOFOLLOW|AT_SYMLINK_NOFOLLOW                      # 已禁止跟随符号链接
→ lstat\(                                                # 使用 lstat 不跟随
→ unlink\(                                               # unlink 操作 symlink 本身
→ O_CREAT.*O_EXCL|O_EXCL.*O_CREAT                      # 独占创建原子操作
→ openat\(.*O_NOFOLLOW                                  # openat + O_NOFOLLOW 安全模式
```
